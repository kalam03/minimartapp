...
import { ReportsService, PurchaseSummaryDto } from '../../services/reports.service';
...
@Component({
  ...
  styleUrls: ['./purchase-summary-report.component.css']
})
export class PurchaseSummaryReportComponent implements OnInit {
  ...
  loadReport(): void {
    this.isLoading = true;
    this.errorMsg  = '';
    this.reportsService.getPurchaseSummary(this.fromDate, this.toDate).subscribe({
      next: (res) => {
        if (res.success) {
      ...
      error: (err) => {
        this.errorMsg  = err?.error?.message || this.t('purchaseSummary.errors.load');
        this.isLoading = false;
        this.cdr.detectChanges();
      }
    });
  ...
  exportPdf(): void {
    ...
    this.reportsService.getPurchaseSummaryPdf(this.fromDate, this.toDate).subscribe({
      next: (blob) => {
        downloadBlob(blob, `Purchase-Summary_${this.fromDate}_to_${this.toDate}.pdf`);
        this.isExporting = false;
        this.cdr.detectChanges();
      },
      error: (err) => {
        this.errorMsg  = err?.error?.message || this.t('purchaseSummary.errors.pdf');
        this.isExporting = false;
        this.cdr.detectChanges();
      }
    });
