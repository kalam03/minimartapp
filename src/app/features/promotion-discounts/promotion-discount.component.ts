...
export class PromotionDiscountComponent implements OnInit {
  ...
  // Picked in the form but uploaded separately, after the discount itself is created/updated — the
  // upload endpoint needs an existing PromotionId to name the saved file after (see
  // promotion-discount.service.ts's uploadImage()).
  ...
  // Mirrors product.component.ts's picker: validates type/size, then just previews locally — the
  // actual upload happens after the discount is saved (see save()), since the upload endpoint needs
  // a real PromotionId.
  ...
  // Uploads the given file (if any) for the given promotion id, then refreshes the grid either way —
  // takes the file as a parameter since resetForm() (called before this) clears selectedImageFile.
  private finishSaveWithImage(promotionId: number, file: File | null): void {
    if (!file) {
      this.load();
      return;
    }
    this.service.uploadImage(promotionId, file).subscribe({
      next: () => this.load(),
      error: (err: any) => {
  ...
  save(): void {
    ...
    obs.subscribe({
      next: (res: any) => {
        ...
        const savedId = editingId ?? res?.data?.promotionId;
        this.resetForm();
  ...
  edit(p: PromotionMaster): void {
    ...
    this.imagePreviewUrl = resolveMediaUrl(p.imageUrl);
    this.validationErrors = {};
